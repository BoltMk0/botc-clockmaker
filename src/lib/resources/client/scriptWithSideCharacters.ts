...
export function withSideCharacters(script: ScriptWithCharacters, all: Character[]): ScriptWithCharacters {
...
export async function fetchAllCharacters(): Promise<Character[]> {
    return fetch('/api/characters').then(r => r.ok ? r.json() as Promise<Character[]> : []).catch(() => [] as Character[]);
}
...
export async function fetchScriptWithSideCharacters(scriptId: string): Promise<ScriptWithCharacters | null> {
    ...
    return withSideCharacters(script, all);
}
