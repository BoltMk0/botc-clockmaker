...
<svg bind:clientWidth={clientWidth} bind:clientHeight={clientHeight} width="{width}" height="{height}" preserveAspectRatio="none" style="transform: rotate({rotation}deg); transform-origin: 50% 50%; {style}">
    ...
    <BuntingFlag color={`hsl(from ${theme.buntingColorBase} calc(h - 15 + ${((i+2)%3) * 30}) calc(20 + s - ${((i+2)%3) * 20}) calc(30 + l * ${((i+2) % 3) * 0.25}))`} x={(i+0.5) * (clientWidth/(numBunting+0.5))} y={1} width={flagWidth} height={clientHeight}/>
    {/each}
</svg>