...
<div style="height: 100%;  width: 100%; overflow: hidden; position: relative; opacity: {otherClockNotificationPresent && !showInstructionText ? 0.6 : 1}; transition: opacity 0.5s ease;">
    ...
    {:else if shownDisplayMode === "clocktower3d"}
        ...
        <ClocktowerScene clockId={model.id} progress={progress} totalTime={model.duration} dayNumber={model.day} playerCount={model.playerCount} style="top: 0; position: absolute;"/>
        {#if showClockNames ?? appSettings.showClockNames}
        <div class="clock-name-title dumbledore-font" style="font-size: {shownSize/10}px; bottom: 0;">{model.name}</div>
        {/if}
