<script lang="ts">
    ...
    import bannerTextureUrl from "$lib/assets/TextBanner1.png";
    ...
    const planeWidth = $derived(screenWidth * widthFraction);
    const planeHeight = $derived(planeWidth / aspect);
    const x = $derived(screenWidth * offsetX);
    const y = $derived(visibleHeight * offsetY);
    ...
    $effect(() => {
        ...
        if (hasSubtitle) {
            ...
            const subAreaTop = mainBlockBottom + gapH;
