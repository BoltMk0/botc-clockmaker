...
import { getCharacterImageResource, setCharacterImageResource } from "$lib/resources/server/character-images";
import type { ScrapeResult } from "../common/types";

export async function scrapeIcon(character: Character): Promise<ScrapeResult> {
    console.log("Scraping character icon", character.name);

    if (getCharacterImageResource(character.id)) {
        console.log("Skipping icon scrape for character", character.name, " - image already exists");
